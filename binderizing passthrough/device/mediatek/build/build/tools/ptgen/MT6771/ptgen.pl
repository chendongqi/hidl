...
my $SCAT_FILE = $RootPackage ne "yes" ?
                "$PRODUCT_OUT/$ArgList{PLATFORM}_Android_scatter.txt" :
                "$PRODUCT_OUT/$ArgList{PLATFORM}_Android_scatter_root.txt";
...
=cut
    ...
    $ArgList{MTK_ENABLE_GENIEZONE} = $ENV{MTK_ENABLE_GENIEZONE};
    ...
    $ArgList{SYSTEM_AS_ROOT}             = $ENV{SYSTEM_AS_ROOT};
    $ArgList{MTK_BUILD_ROOT}             = $ENV{MTK_BUILD_ROOT};
    ...
    if ($ArgList{MTK_BUILD_ROOT} eq "yes" && $ArgList{TARGET_BUILD_VARIANT} eq "eng" && -e "vendor/mediatek/internal/system/core/init")
    {
        $RootPackage = "yes";
    }
...
sub ProcessRawPartitionLayoutData
{
    ...
    for ($partition_idx = 0 ; $partition_idx < @partition_layout_process ; $partition_idx++)
    {
        ...
        {
            if ($ArgList{SYSTEM_AS_ROOT} ne "yes" || !-e "vendor/mediatek/internal/system/core/init" || $ArgList{DTBO_UPGRADE_FROM_ANDROID_O} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^gz([12]|_[ab])$/i)
        {
            if ($ArgList{MTK_ENABLE_GENIEZONE} ne "yes")
            {
                $partition_layout_process[$partition_idx]->{Download} = "N";
                $partition_layout_process[$partition_idx]->{OTA_Update} = "N";
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^vbmeta(_[ab])?$/i)
        {
            if ($ArgList{MTK_BOARD_AVB_ENABLE} ne "true")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
...
sub GenYAMLScatFile()
{
    ...
    my $iter;
    my %root_files = (
                      boot => "mtk_root_boot.img",
                      eng_system => "eng_system.img",
                      eng_vendor => "eng_vendor.img"
                     );
    for ($iter = 0 ; $iter < @partition_layout_cooked ; $iter++)
    {
        ...
        if ($RootPackage eq "yes")
        {
            if (exists $root_files{$part->{Partition_Name}}) {
                $Scatter_Info{$part->{Partition_Name}}{is_download} = "true";
                $Scatter_Info{$part->{Partition_Name}}{file_name} = $root_files{$part->{Partition_Name}};
            } else {
                $Scatter_Info{$part->{Partition_Name}}{is_download} = "false";
                $Scatter_Info{$part->{Partition_Name}}{file_name} = "NONE";
            }
        }
    }
    my $Head1 = <<"__TEMPLATE";
