...
def main(argv):
    try:
        opts, args = getopt.getopt(argv, "hc:k:p:b:l:m:H:s:r:", \
                ["help", "prjconfig=", "kconfig=", "project=", "preloader=", \
                "lk=", "md32=", "header=", "subsystem=", "rsc="])
    except getopt.GetoptError:
        Usage()
        sys.exit(2)

    prjConfig, kConfig, project, plConfig, lkConfig, md32Config, header, \
            subsystem, rsConfig = parse_opt(opts)

    if (not prjConfig and not rsConfig) or not project:
    ...
    if rsConfig:
        check_path(rsConfig)
        rsc_option = get_projectConfiguration(rsConfig)
    reCode = 0
    ...
    if kConfig:
        ...
        if rsConfig:
            reCode += run_check_rsc_config(rsc_option, k_option)
    if plConfig:
        check_path(plConfig)
...
def run_check_rsc_config(prjOption, kOption):
    if 'PRODUCT_MTK_RSC_NAMES' in prjOption:
        aList = prjOption['PRODUCT_MTK_RSC_NAMES'].strip().split(' ')
    else:
        return 0
    if 'CONFIG_BUILD_ARM64_DTB_OVERLAY_IMAGE_NAMES' in kOption:
        kName = 'CONFIG_BUILD_ARM64_DTB_OVERLAY_IMAGE_NAMES'
        kString = kOption['CONFIG_BUILD_ARM64_DTB_OVERLAY_IMAGE_NAMES']
    elif 'CONFIG_BUILD_ARM_DTB_OVERLAY_IMAGE_NAMES' in kOption:
        kName = 'CONFIG_BUILD_ARM_DTB_OVERLAY_IMAGE_NAMES'
        kString = kOption['CONFIG_BUILD_ARM_DTB_OVERLAY_IMAGE_NAMES']
    else:
        return 0
    kList = kString.strip('"').strip().split(' ')
    if len(aList) <= 1:
        return 0
    if len(kList) <= 1:
        return 0
    return_code = 0
    if len(aList) != len(kList):
        return_code = 1
    else:
        for k in range(0, len(aList)):
            ...
            if aValue != kValue:
                return_code += 1
    if return_code > 0:
        print >> sys.stdout, "Kconfig Setting: %s" % kString
        print >> sys.stdout, "RuntimeSwitchConfig Setting: %s" % prjOption['PRODUCT_MTK_RSC_NAMES']
        print >> sys.stdout, "*** String ERROR ***: %s not sync with %s in RuntimeSwitchConfig.mk" % (kName, 'PRODUCT_MTK_RSC_NAMES')
    return return_code
        ...
        for h in hdr_map:
            ...
            if hdr_map[h][fo][0] != prjOption[fo]:
                ...
                if prjOption[fo] == "yes":
                    ...
                    # Ignore options in headers from HIFI3 when FO is no.
                    if h.find("/HIFI3_") != -1 or h.find("/adsp_lib/") != -1:
                        print >> sys.stdout, "Ignored HIFI3 header %s" % h
                        continue
                    is_synced = -1
                    bad[fo].append(tuple([h, hdr_map[h][fo]]))
            else:
...
Usage:
    ...
    -r, --rsconfig=:   the runtime switch configuration path.
