...
ifdef CUSTOM_MODEM
  ...
  $(foreach f,$(MTK_MODEM_MODULE_MAKEFILES),\
    $(if $(strip $(MTK_MODEM_APPS_SEPOLICY_DIRS)),,\
      $(eval MTK_MODEM_APPS_SEPOLICY_DIRS := $(wildcard $(patsubst %/Android.mk,%/sepolicy/p0,$(f))))\
    )\
  )
...
$(shell mkdir -p $(OUT_DIR)/target/product && ln -sfT $(TARGET_DEVICE) $(OUT_DIR)/target/product/$(MTK_TARGET_PROJECT))
endif
endif
